...
\citep{Barger2019, BargerOSF2025} (\url{https://osf.io/py5eb/}). The dataset comprises 50 four-hour daytime recordings from
...
5,760 rows per recording). All loading operations are handled by
\texttt{scripts/utils/data\_loading.py}, a shared utility module used by
every script in the pipeline. This module wraps
\texttt{accusleepy.fileio.load\_recording} (for EEG/EMG signals) and
\texttt{accusleepy.fileio.load\_labels} (for label arrays), returning
standard 1-D float64 and int64 NumPy arrays respectively. Centralising all
I/O in this module ensures consistent loading behaviour across all pipeline
stages.
...
calibration set; 5,400 per recording). The following metrics were
...
detected. The maximum observed Wake proportion across all recordings was
53.5\%, the maximum REM was 15.3\%, and the minimum NREM was 38.2\%, all of
which fall comfortably within the physiologically plausible bounds.
...
epochs per recording. The per-recording low-confidence count had a median of
8 epochs (range: 1--28). Per-recording low-confidence epoch lists are
available in the \texttt{low\_confidence\_epochs/} directory for manual review
if desired. No recordings were excluded from the analysis.
...
its 5 recordings. The three stages accounted for $34.53 \pm 5.16\%$ (Wake),
$54.64 \pm 4.00\%$ (NREM), and $10.83 \pm 1.37\%$ (REM) of total recording
...
across animals. Mean bout durations were $41.5 \pm 10.4$\,s (Wake),
$62.9 \pm 7.0$\,s (NREM), and $76.4 \pm 11.1$\,s (REM; mean $\pm$ SD
across 10 animals).
